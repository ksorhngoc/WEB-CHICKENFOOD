<?php 
    ...
    if(($run->num_rows) < 1 ){ ?>
        <center><h1 style="color: red"><b> Không có đơn hàng chờ lấy!</b></h1></center>
    <?php
    }
    else {
?>
<center><table width="80%" border="2" style="border-collapse: collapse; margin: 20px; background: #FFE4B5">
  ...
  <td><a href="index.php?action=chiTietDonHang&ma=<?php echo $row['maDH']?>&trang=donHangChoLay"><button name = "chiTietDH"style="width: 100px ; height: 30px ; border: 2px solid; margin: 30 10; background:#CC0000;color: white" >Xem chi tiết</button></a></td>
		<form action= "index.php?action=donHangChoLay&ma=<?php echo $row['maDH']?>"" method="post">
        <td><button name="xacNhanLay" style="width: 100px ; height: 30px ; border: 2px solid; margin: 30 10; background:#CC0000; color: white" >Xác nhận</button></td>
    <?php } ?>
  </tr>
    </table>
<?php } 
...
<?php 
  ...
  if(isset($_POST["xacNhanLay"])){
  $ma_dh = $_GET['ma'];
  $sql = "UPDATE donhang set trangThai = '5' where maDH = '$ma_dh' ";
  $run=$conn->query($sql);
}
?>