<?php 
    ...
    $run=$conn->query($sql);
    if(($run->num_rows) < 1){ ?>
        <center><h1 style="color: red"><b> Không có đơn hàng đã hủy!</b></h1></center>
    <?php
    }
    else {
?>
<center><table width="80%" border="2" style="border-collapse: collapse; margin: 20px; background: #FFE4B5">
  ...
  ?>
  <tr align="center">
   	<td><a style="text-decoration:none; color: black" href="trangChu.php?action=chiTietDonHang&ma=<?php echo $row['maDH']  ?>"><?php echo $row['maDH']  ?></a></td>
	<td><?php echo $row['tenKH'] ?></td>
	<td><?php echo $row['SDT'] ?></td>
	<td><?php echo $row['diaChi'] ?></td>
	<td><?php echo $row['SoluongSP'] ?></td>
	<td><?php echo $row['tongTien'] ?></td>
  <td><a href="index.php?action=chiTietDonHang&ma=<?php echo $row['maDH']?>&trang=donHangDaHuy"><button style="width: 100px ; height: 30px ; border: 2px solid; margin: 30 10; background:#CC0000;color: white" >Xem chi tiết</button></a></td>
    <?php } ?>
  </tr>
    </table>
<?php } 
