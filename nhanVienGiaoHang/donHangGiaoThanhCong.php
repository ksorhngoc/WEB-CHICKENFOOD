<?php 
    ...
    if(($run->num_rows) < 1){ ?>
        <center><h1 style="color: red"><b> Không có đơn hàng giao thành công!</b></h1></center>
    <?php
    }
    else {
?>
<center><table width="80%" border="2" style="border-collapse: collapse; margin: 20px; background: #FFE4B5">
  ...
  <td><a href="index.php?action=chiTietDonHang&ma=<?php echo $row['maDH']?>&trang=donHangGiaoThanhCong"><button style="width: 100px ; height: 30px ; border: 2px solid; margin: 30 10; background:#CC0000;color: white" >Xem chi tiết</button></a></td>
    <?php } ?>
  </tr>
    </table>
<?php } 
